...
void ListaCircular<T>::ordenar(Compare comp) {
    if (listaVacia()) return;
    Ordenador<NodoCircular<T>>::ordenarPorSeleccionCircular(primero, comp);
    guardarEnArchivo();
}

template <typename T>
void ListaCircular<T>::guardarCaracteresEnArchivoCircular(const std::string& nombreArchivo, const std::string& atributo) {
    if (listaVacia()) return;
    ...
    do {
        if (atributo == "cedula") {
            archivo << Ordenador<NodoCircular<T>>::ordenarCaracteres(aux->getNumCedula()) << "|" << aux->getPrimerNombre() << "|" << aux->getApellido() << std::endl;
        } else if (atributo == "nombre") {
            archivo << aux->getNumCedula() << "|" << Ordenador<NodoCircular<T>>::ordenarCaracteres(aux->getPrimerNombre()) << "|" << aux->getApellido() << std::endl;
        } else if (atributo == "apellido") {
            archivo << aux->getNumCedula() << "|" << aux->getPrimerNombre() << "|" << Ordenador<NodoCircular<T>>::ordenarCaracteres(aux->getApellido()) << std::endl;
        }
        aux = aux->getSiguiente();
    } while (aux != primero);
...
template <typename T>
void ListaCircular<T>::ordenarCaracteresEnAtributo(const std::string& atributo) {
    Ordenador<NodoCircular<T>>::ordenarCaracteresEnAtributoCircular(primero, atributo);
    guardarCaracteresEnArchivoCircular("usuariosCaracteres.txt", atributo);
}
...
template <typename T>
void ListaCircular<T>::cargarCaracteresDesdeArchivo() {
    ifstream archivo("usuariosCaracteres.txt");
    if (!archivo.is_open()) return;
