...
#include <iostream>
#include <conio.h>
#include <cctype>  // Para tolower y toupper
...
void ListaSimple<T>::insertar(T numcedula, T primerNombre, T apellido) {
    if (cedulaRepetida(numcedula)) {
        cout << "El número de cédula ya existe. No se puede insertar." << endl;
        return;
    }
...
void ListaSimple<T>::buscar(T numcedula) {
    ...
    while (aux != nullptr) {
        if (aux->getNumCedula() == numcedula) {
            cout << "El número de cédula " << numcedula << " corresponde a " << aux->getPrimerNombre() << " " << aux->getApellido() << endl;
            return;
        }
        aux = aux->getSiguiente();
    }
    cout << "El número de cédula " << numcedula << " no se encuentra en la lista" << endl;
}
...
void ListaSimple<T>::eliminar(T numcedula) {
    ...
    while (actual != nullptr) {
        if (actual->getNumCedula() == numcedula) {
            ...
            delete actual;
            cout << "El usuario con el número de cédula " << numcedula << " ha sido eliminado" << endl;
            guardarEnArchivo();
            return;
        }
        anterior = actual;
...
template <typename T>
void ListaSimple<T>::cargarCaracteresDesdeArchivo() {
    ifstream archivo("usuariosCaracteres.txt");
    if (!archivo.is_open()) return;

    string linea;
    NodoSimple<T>* ultimo = nullptr;
    while (getline(archivo, linea)) {
        istringstream iss(linea);
        T numcedula, primerNombre, apellido;
        getline(iss, numcedula, '|');
    ...
    while (true) {
        ...
        if (isdigit(c) && dato.length() < 2) {
            ...
            if (opcion >= minOpcion && opcion <= maxOpcion) {
                ...
                cout << "\n>> Opción inválida. Ingrese nuevamente." << endl;
                dato = "";
                cout << msj << ": ";
            }
        }
    ...
    while (true) {
        ...
        if (isdigit(c) && c != ' ') {
            ...
            if (dato.length() == 10) {
                ...
                cout << "\n>> Debe ingresar exactamente 10 números. Intente nuevamente." << endl;
                dato = "";
                cout << msj << ": ";
            }
        }
...
void ListaSimple<T>::guardarCaracteresOrdenadosEnArchivo(const std::string& atributo) {
    ...
    while (aux != nullptr) {
        if (atributo == "cedula") {
            archivo << Ordenador<NodoSimple<T>>::ordenarCaracteres(aux->getNumCedula()) << "|" << aux->getPrimerNombre() << "|" << aux->getApellido() << endl;
        } else if (atributo == "nombre") {
            archivo << aux->getNumCedula() << "|" << Ordenador<NodoSimple<T>>::ordenarCaracteres(aux->getPrimerNombre()) << "|" << aux->getApellido() << endl;
        } else if (atributo == "apellido") {
            archivo << aux->getNumCedula() << "|" << aux->getPrimerNombre() << "|" << Ordenador<NodoSimple<T>>::ordenarCaracteres(aux->getApellido()) << endl;
        }
        aux = aux->getSiguiente();
    }
    archivo.close();
