...
void menuListaSimple(ListaSimple* lista, ListaDoble* listaDoble, ListaCircular* listaCircular) {
    ...
    while (opcion != 6) { // Cambiar a 7 opciones
        clear();
        printw("MENU PRINCIPAL LISTA SIMPLE\n");
        for (int i = 1; i <= 6; ++i) { // Cambiar a 7 opciones
            if (i == opcion) {
                attron(A_REVERSE);
                printw("%d. ", i);
                attroff(A_REVERSE);
...
int main() {
    ...
    bool cargarDatos = listaSimple->cargarDatosPersona();
    bool cargarDatosDoble = listaDoble->cargarDatosPersona();
    bool cargarDatosCircular = listaCircular->cargarDatosPersona();
    while(true){
